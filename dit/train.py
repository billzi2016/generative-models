...
本脚本使用 Diffusers DiTTransformer2DModel 作为 denoiser backbone，并沿用标准
DDPM 噪声预测目标。DiT 与 DDPM 的主要差别在网络结构：这里用 Transformer 处理
VAE latent patch，而不是 U-Net。
"""
...
def parse_args() -> TrainConfig:
    parser = argparse.ArgumentParser(description="训练 latent DiT")
    parser.add_argument("--latents-h5", default=str(DEFAULT_LATENTS_H5))
    parser.add_argument("--output-dir", default="dit/runs/latent_dit")
...
def build_model() -> DiTTransformer2DModel:
    """
    构建 Diffusers DiT。

    sample_size=16 对应 VAE latent 空间；patch_size=2 会得到 8x8 tokens。
    该配置保持工程可训练性，不手写 Transformer。
    """
...
def run_epoch(model, scheduler, dataloader, device, optimizer=None) -> float:
    ...
    for clean_latents in progress:
        ...
        with torch.set_grad_enabled(is_train):
            class_labels = torch.zeros(clean_latents.shape[0], device=device, dtype=torch.long)
            noise_pred = model(noisy_latents, timestep=timesteps, class_labels=class_labels).sample
